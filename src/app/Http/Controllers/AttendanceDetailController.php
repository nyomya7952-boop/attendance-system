...
{
    /**
     * 勤怠詳細画面を表示（未登録日の場合は空フォーム）【一般ユーザー】
     */
    ...
    public function storeByDate(AttendanceDetailStoreRequest $request)
    {
        ...
        $exists = Attendance::where('user_id', $targetUserId)
            ...
            ->first();
        if ($exists) {
            return redirect()->route('attendance.detail', $exists->id);
        }
        ...
        return redirect()->route('attendance.detail', $attendance->id)->with('success', '勤怠データを登録しました。');
    }
}
