...
<div class="stamp-correction-request-list__content">
    ...
    <div class="stamp-correction-request-list__tabs">
        <a href="{{ route('admin.request.list', ['tab' => 'submitted']) }}" class="stamp-correction-request-list__tab {{ ($activeTab ?? 'submitted') === 'submitted' ? 'stamp-correction-request-list__tab--active' : '' }}">
            {{ \App\Enums\AttendanceRequestStatus::SUBMITTED->value }}
        </a>
        <a href="{{ route('admin.request.list', ['tab' => 'approved']) }}" class="stamp-correction-request-list__tab {{ ($activeTab ?? 'submitted') === 'approved' ? 'stamp-correction-request-list__tab--active' : '' }}">
                    ...
                    @foreach($attendanceRequests as $attendanceRequest)
                        <tr class="stamp-correction-request-list__table-row">
                            ...
                            <td class="stamp-correction-request-list__table-cell">
                                <a href="{{ route('admin.request.approve.show', $attendanceRequest->id) }}" class="stamp-correction-request-list__detail-link">詳細</a>
                            </td>
                        </tr>
                    @endforeach
                @endif
